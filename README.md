...
$$
    \operatorname{\hat H} = \left( -\frac{\hbar^{2}}{2m}\frac{\mathrm d^2}{\mathrm d x^2}
    + V(x) \right)
$$
...
O presente trabalho teve como objetivo estudar três diferentes *toy models* para aplicação: poço finito, poço duplo finito, Krönig-Penney contínuo. 

\begin{figure}[H]
\centering
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{potencial_poco.png}
    \caption{Poço finito}
\end{subfigure}
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{potencial_duplo_poco.png}
    \caption{Poço duplo}
\end{subfigure}
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{potencial_kronig_penney.png}
    \caption{Krönig-Penney contínuo}
\end{subfigure}
\caption{\small Imagens ilustrativas das formas de potenciais.}
\label{fig:tres_potenciais}
\end{figure}

\subsubsection{Poço Finito}

O poço finito representa, de forma bem simplificada, um átomo de hidrogênio, pois permite estudar um objeto quântico confinado tal qual um elétron preso ao núcleo \cite{Supriyo}. Ele consiste em um potencial que tem o valor $V_0$ constante para todo $x$, exceto por um trecho que possui valor zero, como mostra a Figura 1 a).

\subsubsection{Poço Duplo Finito}

Se o poço finito representa um átomo de hidrogênio, o poço duplo finito representa uma molécula de $\mathrm H_2^{+}$, usado para modelar o confinamento de um elétron - um objeto quântico - nessa molécula diatômica \cite{Supriyo}. Ele consiste em um potencial que tem o valor $V_0$ constante para todo $x$, exceto por dois trechos nos quais possui valor zero, como mostra a Figura 1 b).

\subsubsection{Krönig-Penney Contínuo}
...
\subsection{Aplicando o Método das Diferenças Finitas}
...
\subsection{Determinando as Condições de Cada Problema}
...
Para modelar o potencial de Krönig-Penney contínuo foi usada uma função cosseno (14). 
...
Essa relação estabelece a periodicidade. Dela retira-se um Hamiltoniano ligeiramente diferente, com exponenciais complexos nos cantos superior direito e inferior esquerdo \cite{Supriyo}.
...
\subsection{Modelagem Computacional em Python}

Com as condições de cada problema determinadas, o passo final foi construir o problema em Python. A primeira etapa para modelagem de cada caso foi a definição das constantes físicas, números de pontos da malha e o passo ($\ell$). Em sequência, foram utilizadas as bibliotecas \textbf{numpy} e \textbf{scipy} para definir os potenciais e construir os operadores $\hat{H}_{df}$. Enfim, eram aplicadas as condições de contorno e feita a solução do problema de autovalor e autofunção também pelo \textbf{scipy}. Os gráficos dos resultados foram apresentados com auxílio do módulo \textbf{matplotlib}.
...
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{estados_poco.png}
    \caption{Poço finito}
\end{subfigure}
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{estados_poco_duplo.png}
    \caption{Poço duplo}
\end{subfigure}
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{bandas_kronig_penney.png}
    \caption{Krönig-Penney contínuo}
\end{subfigure}
\centering\caption{\small Resultados obtidos. Em a) e b) \centering observa-se as funções de onda em seus respectivos \centering níveis energéticos. Em c) observa-se bandas de energia.} 
\label{fig:tres_resultados} 
\end{figure}

Os gráficos mostram o comportamento esperado para cada modelo \cite{Supriyo}. No poço finito é possível identificar versões unidimensionais de orbitais e seus nódulos. No poço duplo é possível identificar sobreposição ímpar e par de orbitais. No modelo de Krönig-Penney é possível identificar a estrutura de bandas e gaps de energia.
